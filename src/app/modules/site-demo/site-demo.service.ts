...
export class SiteDemoService {
    ...
    getSites(): Observable<Site[]> {
        return this.http.get<ApiSite[]>(`${this.api_base}ui-api/locations/sites`).pipe(
    ...
    getHourlyDistribution(siteId: string, startMs: number, finishMs: number): Observable<HourlyDistributionRow[]> {
        ...
        return this.http
            .post<DistributionRawItem[]>(`${this.api_base}ui-api/camera/analytics/distribution`, payload)
            .pipe(
                map(arr => Array.isArray(arr) ? arr : []),
                map(items =>
